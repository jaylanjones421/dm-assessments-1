...
//Code here
let dragonMapper = arr => arr.map(x => x + "...here be dragons");
...
//Code here
let cookieLoversOnly = arr => arr.filter(x => x.favoriteCookie);
...
//Code here
let changeValue = (arr, cb) => arr.forEach(x => cb(x));
...
//Code here
let findProduct = arr => arr.reduce((acc, cur) => acc * cur);
